...
当然在每篇日志后面都会有至少一个代码，这些代码是理解本次内容所必需的，一般的这些代码都是**Python**在**TensorFlow**框架下完成的。

TensorFlow的安装过程。在Windows环境下进行编译安装是很困难的，但是一般TensorFlow程序也不会在windows上完成计算过程。如果只是在windows环境下安装，确保安装了python3。目前TensorFlow支持在python3.5 和python3.6 的环境下直接安装，但是依赖项较多，建议直接安装Anaconda一步到位。安装的时候一定要选择添加**环境变量**！
在准备好了python环境后，找一个网络条件较好的时间（科研要趁早，最好在早上人少的时候进行尝试），准备安装。管理员身份打开cmd，输入以下命令
>pip install tensorflow

或
>pip install tensorflow-gpu

含-gpu的是安装gpu版，TensorFlow的运算在GPU上效果往往比CPU要好很多，当然在TPU上效率更好。
如果选择安装gpu版，还需要安装CUDA Toolkit 9.0 和cuDNN 7.0 ，同时要求显卡支持CUDA 3.0以上，这些东西需要在NVIDIA的官网上确认下载，安装过程也不复杂，可以参考官方给的说明文档。如果我没记错的话按照顺序挨个装一遍就能用了。对于较小的计算量，cpu版也足够使用。
...
对于数量比较小的样本时，人们通常采用正规方程的方式降低预测结果的误差，而对于数量较大（比如超过10000）的样本时，往往会采用梯度下降法进行计算。梯度下降法以后也会单独说明。这里只给出一小段代码。
