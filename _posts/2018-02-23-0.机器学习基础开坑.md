...
至于为什么是这样的，大概之后会专门说一下。这里的$w^T$和$x$是向量，$w^T$又经常被称为是参数。但是我们要明白，输入的$$x$$和$$y$$并不一定是一维的。多线性回归的本质和单线性回归是一样的。这个时候任务**T**就是根据预测的$$x$$和$$y$$关系来从$$x$$预测$$y$$，对于性能度量**P**，为了确保能尽可能评价模型性能，，我们常采用均方差函数来评价
