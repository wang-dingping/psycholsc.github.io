...
全频复用固定卫星系统的预编码已达到研究成熟水平，并且考虑到其提供的潜在收益，正在考虑由运营商实施。该研究调查了预编码在全频率重用移动交互式多波束卫星系统中的使用。它涉及整合各种移动场景的信道模型 - 慢速游牧，海上和海上低海拔 - 跨越终端速度范围。基于信道模型，研究了发射机处的过时CSI，其产生了对使用可用预编码技术的障碍。由于同样影响所需和干扰信号的卫星通信的衰落性质，已经表明预编码器矩阵对过时的幅度敏感而不是对信道的相位敏感。在移动L波段类型的系统上评估预编码技术的性能，并且表明在执行完美速率分配并使用固定预编码矩阵的情况下，所考虑的场景不会遭受遍历的和速率降低。相反，在速率分配基于SINR的延迟版本的情况下，观察到和速率降级。尽管存在性能损失，但所提出的技术可以在给定所有考虑的场景的特定中断目标的情况下增加平均吞吐量。
