...
>或写作
>
>$$loss^t=-\sum\limits_{i} y_i\: log(\hat y_i^t)$$
>
...
不失普遍性，假设采用激活函数$$f(x)​$$为`tanh`，采用输出层激活函数$$g(x)​$$为`softmax`，损失函数为交叉熵。以下开始反向传播推导过程。反向传播（Back Propagation）实际上是根据已知输出进行系统修正的过程，一般的需要通过链式法则进行求导计算，根据导数（梯度）来决定每一个参数应该向什么方向做多大步进的修正。经过不断的迭代修正，就能得到正确结果。最重要的步骤就是求导。
...
$$loss=-\sum\limits_{t}log(\hat y^t_i)= -\sum\limits_{t}log\left(Softmax( W_{hy}h^t + b_{y} )\right) ​$$

$$Softmax=\frac{e^{x_i}}{\sum\limits_j e^{x_j}}=\frac{e^{x_i}}{K}$$

可以看出`Softmax`操作给予了一个指数归一化，此时损失函数进一步写作

> 此处存疑有空算

$$loss=-\sum\limits_{t} (W_{hy}h^t+b_y)+tlogK$$

其中

$$K=\sum\limits_j e^{x_j}$$
