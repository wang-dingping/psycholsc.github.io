...
>
>另外在`Softmax`激活函数输出后常用的损失函数实际上是**对数**损失函数，因为可以降低Softmax的计算难度。表达式为
>
>$$loss^t = -log(\hat y_i^t)​$$



不失普遍性，假设采用激活函数$$f(x)$$为`tanh`，采用输出层激活函数$$g(x)$$为`softmax`，损失函数为交对数似然函数（采用`Softmax`时常用对数似然降低求导的难度）。

> 说到`Softmax`就不得不说一说它的由来。这个在吴恩达的课程中实际上做了详细的介绍，此处稍微整理一下相关内容。
>
...
> 
>
> 由于这属于机器学习的范畴了，因此在这里不做详细说明。对于多分类模型，往往是采用`Softmax`函数处理结果。
>
...
>
> 其中$$\eta$$被称为分布的**特性参数**或**自然参数**，$$T(y)$$是充分统计量，$$a(\eta)$$是对数分割函数，使$$e^{-a(\eta)}$$成为一个归一化常数，规定其取值可以令$$\sum p(y;\eta)=1$$。规定了$$T,a,b$$的模型可以表示一族分布，在改变$$\eta$$时分布随之在族内改变。伯努利分布（二项分布）和高斯分布都是指数函数族内的一种特殊情况。例如二项分布
>
...
> 
>
> 对于多分类问题，假设$$y​$$可以分为$$k​$$种不同类别，使用GLM建立模型。
>
> 首先为了参数化分类结果，目前常用的手段是采用一个列向量作为输出结果，描述方法为
>
> $$\hat y=\left[ \begin{matrix} \hat y_1^t  \\ \hat y_2^t   \\ ... \\ ... \\ \hat y_k^t  \end{matrix} \right]​$$
>
> 里面的数值描述的是分类器输出对应结果的概率。但是直接计算的结果并不能保证这些数字符合概率分布的特点（例如求和未必是1，范围并不在指定内），且存在冗余。那么选取$$k-1​$$个分类进行独热编码，第$$k​$$类就可以表示为全$$0​$$编码。
>
> 
>
> 
...
以下开始反向传播推导过程。反向传播（Back Propagation）实际上是根据已知输出进行系统修正的过程，一般的需要通过链式法则进行求导计算，根据导数（梯度）来决定每一个参数应该向什么方向做多大步进的修正。经过不断的迭代修正，就能得到正确结果。最重要的步骤就是求导。
...
可以计算，根据上面的计算，总损失函数写作

$$loss=-\sum\limits_{t}log(\hat y^t_i)​$$

其中$$i​$$代表这一组输出的参考独热输出中为$$1​$$那一项的索引，和前面的含义一致。
