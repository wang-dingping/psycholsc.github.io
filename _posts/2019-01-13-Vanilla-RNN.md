...
>
>另外在`Softmax`激活函数输出后常用的损失函数实际上是对数损失函数，因为可以降低Softmax的计算难度。表达式为
>
>$$loss^t = -log(\hat y_i^t)​$$



不失普遍性，假设采用激活函数$$f(x)$$为`tanh`，采用输出层激活函数$$g(x)$$为`softmax`，损失函数为交对数似然函数（采用`Softmax`时常用对数似然降低求导的难度）。以下开始反向传播推导过程。反向传播（Back Propagation）实际上是根据已知输出进行系统修正的过程，一般的需要通过链式法则进行求导计算，根据导数（梯度）来决定每一个参数应该向什么方向做多大步进的修正。经过不断的迭代修正，就能得到正确结果。最重要的步骤就是求导。
