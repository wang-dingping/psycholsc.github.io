...
本文提供的算法构成了CE核心，可用作下一代航天器和卫星中空间通信系统发展的基线。 提出了一种概念验证解决方案，通过RL实现多目标性能。 它依赖于利用NN开发和探索无线电参数集来动态改变频道。

所提出的解决方案包括用于探索在RL的探索阶段期间提供性能控制的动作参数的深度神经网络的集合，以及用于预测某个已知性能目标的无线电参数的浅NN集合的阵列。 所提出的算法使得能够在连续动作状态宇宙中灵活地进行若干无线电参数自适应，而不是改善特定度量的性能，同时保持随着时间的推移而固定的存储器要求。 基于由系统操作员定义的所选通信任务配置来监视整体性能。

提供了关于所提出的解决方案的计算成本，执行时间及其性能准确性的权衡分析。 仿真结果表明，与理想的解决方案相比，可以实现非常低的性能误差。 所有示例任务专业实现了平均准确率高于80％。 这些结果有望成为未来空间通信系统CE研究的基准。
