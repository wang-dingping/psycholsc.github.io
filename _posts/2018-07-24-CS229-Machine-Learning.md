...
常用的代价函数很多，平方函数用的很多，即

$$cost_i=(h_\theta(x_i)-y_i)^2$$

至于这里为什么要使用$$(\Delta y)^2$$作为损失函数（Square Error），后面会做介绍。
...
这种形式的代价函数有时也被叫做平方误差函数，其前面的系数对结果往往没有直接影响（但是在程序设计过程中有可能因为系数大小问题而出现报错）。除此之外，实际上还有许多其他代价函数可以在机器学习的过程中进行选择，例如交叉熵（详见《信息论》）。
