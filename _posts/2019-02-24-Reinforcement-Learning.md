...
简单介绍这个所谓的抉择问题。UCB介绍这个问题时采用的例子是，我们每天去吃饭都需要选择餐厅。假设我们面前有两个选择，一个是我们经常去的餐厅，另一个是我们从没有去过的餐厅。对于我们每天都去的餐厅，我们对于吃什么东西是非常了解的，因此我们一般来讲会吃的比较满意；但是对于从未去过的餐厅，这里可能并不好吃，但是也有可能有更佳选择。我们一直选择其中的一个都不是最佳决策，因此需要在两个选项之间权衡。相似的，
...
如果环境有关的信息都已经全部知道了，我们就可以找到一个最有效的策略，即使是采用暴力仿真的方法，更不用说采用其他的“智能算法”了。所谓的`dilemma`正是信息不完全导致的，我们需要获取足够的信息，以便在控制风险的条件下找到最合适的决策方案。其中`exploitation`就是我们采取已知的最优决策方案，而`exploration`则是我们冒险收集环境未知选项的信息。最优的长期决策方案可能伴随短期的牺牲，例如`exploration`可能失败，但是这也告诉我们以后不再采用相关方案，从而提高了长期收益。
